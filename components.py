...
def plot_el_curves(El, config_dict):
    ...
    fig_prod=plt.figure(dpi=600)
    #fig_prod=plt.gcf().number
    # plt.title('Hydrogen production')
    plt.xlabel('Power input[MW]')
...
def obj_func(El, Wind, SMR, Prices, df, df_H2, y_off_on, z_on, delta_t):
    
    obj_val =   (
                + sum((Prices['pi_e'] + Prices['nr_tariff'] + Prices['C_var']) * df['p_purchased[MW]'] * delta_t)
                + sum((Prices['pi_ppa'] + Prices['nr_tariff']) * Wind['P_w'] * delta_t)
                #EL degradation
                + sum(sum((El['degr_cold']*El['C_stack']/El['lifetime'])*np.concatenate(y_off_on)))
                + sum(sum((El['degr']*El['C_stack']/El['lifetime'])*np.concatenate(z_on)*delta_t))
                #SMR
                + sum(Prices['pi_ng'] * df['q_ng_smr']) # NG Fuel Cost
                + sum(SMR['C_su'] * df['s_smr_su'])                # Startup Cost
                #Earnings
                - sum(Prices['pi_e'] * df['p_w_sold[MW]'] * delta_t)
                # --- Earnings from Premiums ---
                - sum(Prices['prem_green'] * df_H2['H2_green'])
                - sum(Prices['prem_grid'] * df_H2['H2_lowC'])
                )
